...
X = data.loc[:, ['Open','High','Low','Shares Traded','Turnover (Rs. Cr)']].values
y = data['Close'].values
print(type(X))
print(X)
print(y)

#Estimate MSE using cress-validation in Linear Regression
...
#Evaluate the GA class appraoch using LR estimator
#Plot the generation scores
sel = GeneticSelector(estimator=LinearRegression(), n_gen=50, size=200, n_best=40, n_rand=40, n_children=5, mutation_rate=0.05)
sel.fit(X, y)
sel.plot_scores()


#MSE using GA approach
